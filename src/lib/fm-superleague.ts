...
export function computeClubChampions(standings: StandingRow[]): ChampRow[] {
  ...
  for (const s of slStandings(standings)) {
    const tag = `${s.season_year} (Div ${s.division_num ?? "?"})`;
...
export function computeCoachChampions(standings: StandingRow[], coaches: CoachRow[]): ChampRow[] {
  ...
  for (const s of slStandings(standings)) {
    ...
    if (isC(s.info)) { r.c++; r._C.push(`${s.club_name} — ${s.season_year}`); }
    if (isP(s.info)) { r.p++; r._P.push(`${s.club_name} — ${s.season_year} (Div ${s.division_num ?? "?"})`); }
    if (isD(s.info)) { r.d++; r._D.push(`${s.club_name} — ${s.season_year} (Div ${s.division_num ?? "?"})`); }
    map.set(tr.name, r);
  }
  return [...map.values()]
