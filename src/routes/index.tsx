    ...
    <Card className={`relative overflow-hidden card-glow card-glow-hover ${accent ? "border-gold/40" : ""}`}>
      ...
      <CardContent className="relative">
        ...
        <div className="mt-3 flex gap-5 text-sm">
          <div>
            <p className="font-bold tabular-nums">{Math.round(entry?.weighted ?? 0).toLocaleString("pt-PT")}</p>
        ...
        {entries.slice(0, 8).map((e, i) => {
          ...
          return (
            <div key={e.name} className="flex items-center gap-3 py-1.5 text-sm border-b border-border/40 last:border-0">
              ...
              <span className="font-semibold tabular-nums w-16 text-right">{Math.round(e.weighted).toLocaleString("pt-PT")}</span>
            </div>
          );
        })}
      </CardContent>
